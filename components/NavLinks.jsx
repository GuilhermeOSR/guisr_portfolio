...
const NavLinks = ({containerStyles}) => {
    ...
    return (<ul className={containerStyles}>
        {links.map((link, index) => {
            ...
            return (
                <Link href={link.path} key={index} className={`relative text-lg uppercase text-white
                    ${isActive && `after:content-[''] after:block after:absolute after:left-0
                after:top-1/2 ${lineWidth} after:h-[4px] after:bg-accent after:-translate-y-1/2 after:z-0`}`}>
